...
Each file is a complete `DraftResponse` for a representative input. Together they
demonstrate both the happy path and the safety system catching real failures.
...
| [`assignment_output.mock.json`](assignment_output.mock.json) | Assignment patient summary, deterministic mock LLM | `true` | Same input, reproducible output for CI/tests. No API key required. |
...
```bash
# 1. Happy path (real OpenAI)
LLM_PROVIDER=openai OPENAI_API_KEY=sk-... \
  curl -X POST http://localhost:8090/api/v1/drafts \
    -H 'content-type: application/json' \
    -d "$(jq -n --arg s "$(cat assignment_input.txt)" '{raw_text:$s}')"

# 2. Happy path (deterministic mock — works without an API key)
python -c "from clinical_ai.app.pipeline import DraftPipeline; \
           import json; \
           print(json.dumps(DraftPipeline().process(open('assignment_input.txt').read()).model_dump(mode='json'), indent=2))"

# 3. Privacy gate trigger
curl -X POST http://localhost:8090/api/v1/drafts \
  -H 'content-type: application/json' \
  -d '{"raw_text":"Patient name: Jane Smith\nMRN: ABC-123-456\nEmail: jane.smith@example.com\n..."}'

# 4. Unsafe-directive trigger — see tests/test_safety.py for the regex set
```
